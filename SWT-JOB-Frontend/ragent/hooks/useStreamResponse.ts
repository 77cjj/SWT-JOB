...
export interface StreamHandlers {
  ...
  onResources?: (payload: ResourcesPayload) => void;
  ...
  const dispatchEvent = () => {
    ...
    switch (eventName) {
      ...
      case "reject":
        ...
        break;
      case "resources":
        handlers.onResources?.(payload as ResourcesPayload);
        break;
      case "title":
...
async function extractResponseErrorMessage(response: Response): Promise<string | null> {
  try {
...
async function streamWithRetry(
  ...
  while (attempt <= retryCount) {
    try {
      ...
      if (!response.ok) {
        const backendMessage = await extractResponseErrorMessage(response);
        throw new Error(backendMessage || `SSE 请求失败（${response.status}）`);
      }
