...
export function IngestionPage() {
  ...
  useEffect(() => {
    ...
    void router.replace("/admin/ingestion?tab=pipelines", undefined, { shallow: true });
  }, [tabParam, router.isReady]);

  const handleTabChange = (next: "pipelines" | "tasks") => {
    setActiveTab(next);
    void router.replace(`/admin/ingestion?tab=${next}`, undefined, { shallow: true });
  };
  ...
  const buildNodesPayload = (sourceNodes: PipelineNodeForm[]) => {
    ...
    for (const node of sourceNodes) {
      ...
      try {
        ...
        const parsedCondition = parseCondition(node.condition);
        condition =
          parsedCondition && typeof parsedCondition === "object" && !Array.isArray(parsedCondition)
            ? (parsedCondition as Record<string, unknown>)
            : undefined;
      } catch (error) {
