...
# Compatible with Python 3.6+ (ECS 上常见 3.6；避免 PEP563/PEP604 语法)
import argparse
import json
import sys
    ...
    for code, name, slug, examples, so in transport_leaves:
        nodes.append(
            node(
                ...
                description="交通出行：{0}".format(name),
                examples=examples,
                prompt_snippet="对应 /docs/transport/{0}".format(slug),
                top_k=6,
                sort_order=so,
            )
        )
...
def stable_id(index):
    ...
    return "82{0:017d}".format(index)


def resolve_kb(n, main_kb, deals_kb, main_col, deals_col):
    kind = n["kb"]
...
def generate_sql(nodes, main_kb=None, deals_kb=None, main_col=None, deals_col=None, disable_demo=False):
    lines = [
    ...
    for i, n in enumerate(nodes, start=1):
        ...
        lines.append(
            ...
            ") VALUES ("
            "{nid}, {kb}, {code}, {name}, "
            "{level}, {parent}, {desc}, {examples}, "
            "{coll}, {top_k}, NULL, {kind}, "
            "{snippet}, {template}, NULL, "
            "{sort_order}, {enabled}, 'seed-swt-intent', 'seed-swt-intent', "
            "CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0"
            ");".format(
                nid=sql_quote(nid),
                kb=kb_sql,
                code=sql_quote(n["intent_code"]),
                name=sql_quote(n["name"]),
                level=n["level"],
                parent=parent_sql,
                desc=sql_quote(n["description"]),
                examples=sql_quote(examples_json),
                coll=coll_sql,
                top_k=top_k_sql,
                kind=n["kind"],
                snippet=sql_quote(n["prompt_snippet"]),
                template=sql_quote(n["prompt_template"]),
    ...
    lines += [
        ...
        "COMMIT;",
        "",
        "-- nodes={count} missing_topic_kb={missing}".format(
            count=len(nodes), missing=missing_topic_kb
        ),
    ]
    if missing_topic_kb:
        lines.append(
