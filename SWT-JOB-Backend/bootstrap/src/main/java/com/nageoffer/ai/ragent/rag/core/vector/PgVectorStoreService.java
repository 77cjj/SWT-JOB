...
public class PgVectorStoreService implements VectorStoreService {
    ...
    public void indexDocumentChunks(String collectionName, String docId, List<VectorChunk> chunks) {
        ...
        String resolvedCollection = StringUtils.hasText(collectionName) ? collectionName.trim() : collectionName;
        ...
        jdbcTemplate.batchUpdate(
                ...
                chunks, chunks.size(), (ps, chunk) -> {
                    ...
                    ps.setString(3, buildMetadataJson(resolvedCollection, docId, chunk));
                    ps.setString(4, toVectorLiteral(chunk.getEmbedding()));
                });

        log.info("批量写入向量到 PostgreSQL，collectionName={}, docId={}, count={}", resolvedCollection, docId, chunks.size());
    }

    @Override
    public void deleteDocumentVectors(String collectionName, String docId) {
        String resolvedCollection = collectionName == null ? null : collectionName.trim();
        // noinspection SqlDialectInspection,SqlNoDataSourceInspection
        int deleted = jdbcTemplate.update(
                "DELETE FROM t_knowledge_vector WHERE metadata->>'collection_name' = ? AND metadata->>'doc_id' = ?",
                resolvedCollection, docId);
        log.info("删除文档向量，collectionName={}, docId={}, deleted={}", resolvedCollection, docId, deleted);
    }
    ...
    public void updateChunk(String collectionName, String docId, VectorChunk chunk) {
        ...
        jdbcTemplate.update(
                ...
                chunk.getChunkId(),
                chunk.getContent(),
                buildMetadataJson(resolvedCollection, docId, chunk),
