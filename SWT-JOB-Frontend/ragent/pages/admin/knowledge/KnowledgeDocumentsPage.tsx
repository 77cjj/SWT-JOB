...
import { useAdminSlugSegments } from "@/hooks/useAdminSlug";
...
export function KnowledgeDocumentsPage() {
  ...
  const segments = useAdminSlugSegments();
  ...
  const handleChunkAllUnchunked = async () => {
    if (!kbId || chunkingAll) return;
    setChunkingAll(true);
    try {
      const size = 100;
      let page = 1;
      ...
      while (page <= pages) {
        ...
        allDocs.push(...(data.records || []));
        pages = data.pages || 1;
        page += 1;
      }

      const targets = allDocs.filter((doc) => {
        const chunkCount = doc.chunkCount ?? 0;
        const status = (doc.status || "").toLowerCase();
        return chunkCount <= 0 && status !== "running";
      });

      if (targets.length === 0) {
        toast.info("没有可分块的未分块文档");
        return;
      }
      ...
      if (failed === 0) {
        toast.success(`已开始 ${success} 个未分块文档的分块任务`);
      } else if (success === 0) {
        toast.error(`批量分块失败，共 ${failed} 个文档触发失败`);
      } else {
        toast.warning(`已开始 ${success} 个，失败 ${failed} 个`);
      }
    ...
    <div className="admin-page">
        ...
        <CardHeader>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-1 flex-wrap items-center justify-end gap-2">
              ...
              <Button variant="outline" onClick={handleChunkAllUnchunked} disabled={chunkingAll}>
                <PlayCircle className="mr-2 h-4 w-4" />
                {chunkingAll ? "批量分块中..." : "全部未分块开始分块"}
              </Button>
              <Select
      ...
      <UploadDialog
        ...
        onSubmit={async (payloads) => {
          ...
          toast.success(payloads.length > 1 ? `成功上传 ${payloads.length} 个文档` : "上传成功");
          setUploadOpen(false);
          setCurrent(1);
          await loadDocuments(1, statusFilter, keyword);
        }}
...
function UploadDialog({ open, onOpenChange, onSubmit }: UploadDialogProps) {
  ...
  const handleSubmit = async (values: UploadFormValues) => {
    ...
    const oversized = files.find((item) => item.size > maxFileSize);
    if (values.sourceType === "file" && oversized) {
    ...
    try {
      if (values.sourceType === "file") {
        const payloads: KnowledgeDocumentUploadPayload[] = files.map((file) => ({
          sourceType: "file",
          file,
          sourceLocation: null,
          scheduleEnabled: false,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>支持本地多文件或远程URL，并配置分块策略</DialogDescription>
        </DialogHeader>
        <Form {...form}>
                  ...
                  >
                    ...
                    {files.length > 0 ? (
                      <>
                        ...
                        <div className="text-sm font-medium text-center px-2">已选择 {files.length} 个文件</div>
                        <div className="max-h-28 w-full space-y-1 overflow-y-auto rounded-md border bg-muted/30 p-2 text-xs">
                          {files.map((file, idx) => (
                        ...
                        <div className="text-xs text-muted-foreground">
                          总大小：{formatSize(files.reduce((sum, item) => sum + item.size, 0))}
                        </div>
                      </>
                    ) : (
                      <>
                        <FileUp className="h-7 w-7 text-muted-foreground" />
                        <div className="text-sm font-medium">拖拽多个文件到此处，或点击选择</div>
