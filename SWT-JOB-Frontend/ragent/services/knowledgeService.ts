...
const pickFirst = (raw: Record<string, unknown>, keys: string[]): unknown => {
...
const normalizeKnowledgeBase = (raw: KnowledgeBase | Record<string, unknown>): KnowledgeBase => {
  const source = raw as Record<string, unknown>;
  const id = asStringOrUndefined(pickFirst(source, ["id", "kbId", "kb_id"])) || "";
  const name = asStringOrUndefined(pickFirst(source, ["name", "kbName", "kb_name"])) || "";
  const embeddingModel =
    asStringOrUndefined(
      pickFirst(source, ["embeddingModel", "embedding_model", "model", "embedding"])
    ) || "";
  const collectionName =
    asStringOrUndefined(
      pickFirst(source, ["collectionName", "collection_name", "collection", "vectorCollection"])
    ) || "";
  ...
  return {
    ...
    embeddingModel,
    collectionName,
    createdBy: asStringOrUndefined(
      pickFirst(source, ["createdBy", "created_by", "creator", "owner"])
    ),
    documentCount: asNumberOrUndefined(
      pickFirst(source, [
        "documentCount",
        "document_count",
        "docCount",
    ...
    createTime: asStringOrUndefined(
      pickFirst(source, ["createTime", "create_time", "createdAt", "created_at"])
    ),
    updateTime: asStringOrUndefined(
      pickFirst(source, ["updateTime", "update_time", "updatedAt", "updated_at"])
    )
  } as KnowledgeBase;
};
...
export const getKnowledgeBases = async (current = 1, size = 200, name?: string): Promise<KnowledgeBase[]> => {
  ...
  return (page?.records || []).map((kb) => normalizeKnowledgeBase(kb));
};
...
export const getKnowledgeBasesPage = async (
  ...
  return {
    ...page,
    records: (page?.records || []).map((kb) => normalizeKnowledgeBase(kb))
  };
};

export const getKnowledgeBase = async (id: string): Promise<KnowledgeBase> => {
  const data = await api.get<KnowledgeBase, KnowledgeBase>(`/knowledge-base/${id}`);
